...
type Body = {
	...
	addon_ids?: string[];
	start_at: string; // ISO
	end_at?: string; // optional if client precomputed
	notes?: string;
	guest?: boolean;
	guest_email?: string;
	guest_first?: string;
...
export async function POST(req: Request) {
	try {
		...
		// downpayment requirement: pay full for now (can be adjusted later)
		...
		const endAt = body.end_at ? new Date(body.end_at) : new Date(startAt.getTime() + durationMin * 60000);
		...
		// record payments row
		await supabase.from("payments").insert({ client_id: clientId, amount_cents: toCharge, status: mapped, square_payment_id: payment?.payment?.id ?? null, source: "booking", source_id: appt.id });

		return NextResponse.json({ ok: true, appointmentId: appt.id, paymentStatus: mapped });
	} catch (e: any) {
